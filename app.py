...
# Veritabanı ayarı
# Eğer Supabase Postgres kullanmak istersen:
...
class Masa(db.Model):
    ...
    siparis_durum = db.Column(db.Integer, default=0)
...
with app.app_context():
    ...
    if Masa.query.count() == 0:
        ...
        db.session.add_all(ornek_masalar)
    if Urun.query.count() == 0:
        ...
        db.session.add_all(ornek_urunler)
    db.session.commit()
...
def management():
    ...
    for masa in masalar:
        ...
        toplam = sum(s.adet * s.urun.fiyat for s in siparisler)
...
def api_masa_detay(masa_id):
    ...
    for siparis in siparisler:
        ...
        toplam += tutar
    return jsonify({
        'masa': {'id': masa.id, 'kat': masa.kat, 'siparis_durum': masa.siparis_durum},
...
def report():
    ...
    toplam_ciro = sum(s.toplam for s in satislar)
    urunler = Urun.query.all()
    envanter_listesi = Envanter.query.all()
    envanter_bilgileri = []
    for envanter in envanter_listesi:
        toplam_satis = sum(s.adet for s in satislar if s.urun_isim == envanter.urun.isim)
        tuketilen = envanter.baslangic_stok - envanter.kalan_stok
        envanter_bilgileri.append({
            'envanter': envanter,
            'tuketilen': tuketilen,
            'yuzde': (tuketilen / envanter.baslangic_stok * 100) if envanter.baslangic_stok > 0 else 0
        })
    return render_template('report.html', satislar=satislar, toplam_ciro=toplam_ciro, urunler=urunler, envanter_bilgileri=envanter_bilgileri)
        ...
        try:
            ...
            if stok_int >= 0:
                ...
                if mevcut_envanter:
                    ...
                    yeni_envanter = Envanter(urun_id=urun_id, baslangic_stok=stok_int, kalan_stok=stok_int)
